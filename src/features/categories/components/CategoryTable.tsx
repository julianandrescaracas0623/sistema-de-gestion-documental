...
function compare(a: CategoryAdminRow, b: CategoryAdminRow, key: SortKey): number {
  ...
  const toggle = (key: SortKey) => {
    setSort((prev) =>
      prev.key === key
        ...
        : { key, dir: key === "created_at" || key === "doc_count" ? "desc" : "asc" }
    );
  };
  ...
  if (rows.length === 0) {
    return (
      <div className="p-10 text-center">
        <p className="text-foreground text-sm font-medium">No hay categorías</p>
        <p className="text-muted-foreground mt-1 text-sm">
          Crea la primera categoría con el formulario inferior.
        </p>
      </div>
    );
  }
        ...
        {sorted.map((row) => (
          <TableRow key={row.id}>
            ...
            <TableCell className="text-right">
              <CategoryRowActions category={row} canUpdate={canUpdate} canDelete={canDelete} />
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
